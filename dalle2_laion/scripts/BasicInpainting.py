...
class BasicInpainting(InferenceScript):
    def run(
        ...
        for i, original_text in range(len(text)):
        ...
        inpaint_images = torch.stack(self._pil_to_torch(images, resize_for_clip=False))
        image_map = self._sample_decoder(
            text=decoder_text,
            image_embed=image_embeddings,
            cond_scale=decoder_cond_scale,
            inpaint_images=inpaint_images, inpaint_image_masks=masks,
